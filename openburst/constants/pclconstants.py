...
SNR_THRESHOLD = float(os.environ.get("SNR_THRESHOLD", 15.0))  # [dB], SNR threshold for detection
DOPPLER_THRESHOLD = float(os.environ.get("DOPPLER_THRESHOLD", 2.0))  # [dB], for detection
MIN_RCS_THRESHOLD = float(os.environ.get("MIN_RCS_THRESHOLD", 150.0))  # [m2], for detection

SENSOR_UPDATE_TIME = int(os.environ.get("SENSOR_UPDATE_TIME", 1)) # [s], update rate for pcl detection

# maximum coherent integration time in [s] (use appropriate values for different signals)
MAX_COHERENT_INTEGRATION_TIME_FM = float(os.environ.get("MAX_COHERENT_INTEGRATION_TIME_FM", 0.5))
...
MIN_BISTATIC_RCS_ONLINE_DETECTION = float(os.environ.get("MIN_BISTATIC_RCS_ONLINE_DETECTION", 10.0)) # [m]
